...
import {
  BarChart3, Boxes, Check, ChevronDown, Coins, Copy, Database, Filter, Gem, Menu,
...
export default function Dashboard() {
  ...
  async function load() {
    ...
    try {
      ...
      setLoadError(error instanceof Error ? error.message : "Không thể tải dữ liệu tài khoản");
  ...
  const filteredIds = useMemo(() => new Set(filtered.map(account => account.id)), [filtered]);
  const selectedInFilter = useMemo(() => filtered.filter(account => selectedIds.has(account.id)), [filtered, selectedIds]);
  ...
  function toggleFiltered() {
    setSelectedIds(current => {
      ...
      const next = new Set(current);
      filtered.forEach(account => next.add(account.id));
      return next;
    });
    setCopied(false);
  }
  ...
  async function copySelectedUsernames() {
    const text = selectedInFilter.map(account => `**${account.username}**`).join(",");
    if (!text) return;
    await navigator.clipboard.writeText(text);
    setCopied(true);
  }

  const hasActiveFilters = Boolean(query || sea !== "All" || fruit !== "All" || ownedFruit !== "All" || status !== "All" || presence !== "All" || minLevel || itemFilter);
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <section className="panel">
        <div className="panel-head"><div><h2>Danh sách tài khoản</h2><p>{filtered.length} trong {accounts.length} tài khoản</p></div><div className={`view-note ${loadError ? "failed" : ""}`}><Database/> {loadError ? "Mất kết nối dữ liệu" : "Supabase/API connected"}</div></div>
        {loadError && <div className="data-error" role="alert"><div><strong>Không tải được tài khoản</strong><span>{loadError}</span></div><button onClick={load}><RefreshCw/> Thử lại</button></div>}
        <div className="toolbar">
          ...
          <div className="filters"><Filter/>
            ...
            {hasActiveFilters && <button className="reset-filters" onClick={resetFilters}><X/> Xóa bộ lọc</button>}
          </div>
        </div>
        <div className="selection-bar">
          <label className="select-all">
            <input type="checkbox" checked={allFilteredSelected} disabled={!filtered.length} onChange={toggleFiltered}/>
            <span>{allFilteredSelected ? "Bỏ chọn tất cả trong bộ lọc" : "Chọn tất cả trong bộ lọc"}</span>
            <b>{selectedInFilter.length}/{filtered.length}</b>
          </label>
          {selectedInFilter.length > 0 && <button className="copy-usernames" onClick={copySelectedUsernames}>
            {copied ? <Check/> : <Copy/>}
            {copied ? "Đã copy username" : `Copy ${selectedInFilter.length} username`}
          </button>}
        </div>

        <div className="table-wrap"><table>
          <thead><tr><th className="check-col"><input type="checkbox" checked={allFilteredSelected} disabled={!filtered.length} onChange={toggleFiltered} aria-label="Chọn tất cả tài khoản trong bộ lọc"/></th><th>Tài khoản</th><th>Tiến độ</th><th>Tiền tệ</th><th>Devil Fruit</th><th>Race / Combat</th><th>Kho nổi bật</th><th>Trạng thái</th></tr></thead>
          <tbody>{filtered.map(a=><tr key={a.id} onClick={()=>setSelected(a)}>
            <td className="check-col"><input type="checkbox" checked={selectedIds.has(a.id)} onClick={event=>event.stopPropagation()} onChange={()=>toggleAccount(a.id)} aria-label={`Chọn ${a.username}`}/></td>
        ...
        </table>{!loading && !loadError && !filtered.length && <div className="empty"><PackageSearch/><strong>{accounts.length ? "Không có tài khoản khớp bộ lọc" : "Chưa có tài khoản"}</strong><span>{accounts.length ? "Bấm Xóa lọc hoặc thay đổi điều kiện lọc." : "Chạy script quét một tài khoản để dữ liệu xuất hiện tại đây."}</span></div>}</div>
      </section>
    </main>
