...
export type StoreDiagnostics = {
  configured: boolean;
  urlHost?: string;
  keyType?: "legacy_jwt" | "secret" | "publishable" | "unknown";
  ok: boolean;
  status?: number;
  message?: string;
};
...
function getKeyType(key: string): StoreDiagnostics["keyType"] {
...
export async function getStoreDiagnostics(): Promise<StoreDiagnostics> {
  ...
  if (!rawUrl || !rawKey) {
    return {
      configured: false,
      ok: false,
      message: `${rawUrl ? "" : "Missing SUPABASE_URL. "}${rawKey ? "" : "Missing SUPABASE_SECRET_KEY/SUPABASE_SERVICE_ROLE_KEY."}`.trim(),
    };
  }

  const urlHost = getUrlHost(rawUrl);
  const keyType = getKeyType(rawKey);
  if (keyType === "publishable") {
    return {
      ...
      keyType,
      ok: false,
      message: "Server dang dung publishable key. Hay dung sb_secret_... hoac legacy service_role JWT.",
    };
  }
  ...
  try {
    ...
    const response = await fetch(`${config.url}/rest/v1/blox_accounts?select=id&limit=1`, {
    ...
    return {
      ...
      message: response.ok ? "Supabase connected" : body.slice(0, 220) || `Supabase returned ${response.status}`,
    };
  } catch (error) {
    return {
      configured: true,
...
export async function upsertAccount(account: BloxAccount) {
  ...
  if (!response.ok) {
    const body = await response.text().catch(() => "");
    throw new Error(`Supabase upsert failed: ${response.status}${body ? ` ${body.slice(0, 220)}` : ""}`);
  }
